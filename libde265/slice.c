      ...
      for (int i = 0; i <= num_ref; i++)
      {
        ...
        if (shdr->chroma_weight_flag[l][i])
          for (int j = 0; j < 2; j++)
          {
            ...
            vlc = Clip3(-128, 127, (vlc - ((128 * shdr->ChromaWeight[l][i][j]) >> shdr->ChromaLog2WeightDenom) + 128));
                                      ...
                                      bool *continueDecoding)
{
  *continueDecoding = false;
  ...
  if (!shdr->dependent_slice_segment_flag)
  {
        ...
        ctx->nal_unit_type != NAL_UNIT_IDR_N_LP)
    {
      ...
      else
      {
        ...
        if (nBits > 0)
          shdr->short_term_ref_pic_set_idx = get_bits(br, nBits);
        else
          shdr->short_term_ref_pic_set_idx = 0;
  ...
  if (pps->tiles_enabled_flag || pps->entropy_coding_sync_enabled_flag)
  {
    ...
    { // TODO: make entry points array dynamic
      add_warning(ctx, DE265_WARNING_SLICEHEADER_INVALID, false);
      return DE265_ERROR_CODED_PARAMETER_OUT_OF_RANGE;
    }
  ...
  case SLICE_TYPE_I:
    ...
    break;
  case SLICE_TYPE_P:
    shdr->initType = shdr->cabac_init_flag + 1 /*shdr->cabac_init_flag ? 2 : 1*/;
    break;
  case SLICE_TYPE_B:
    shdr->initType = 2 - shdr->cabac_init_flag /*shdr->cabac_init_flag ? 1 : 2*/;
    break;
  }
...
#define LOG4(t, d1, d2, d3, d4) log2fh(fh, t, d1, d2, d3, d4)
  ...
  if (!shdr->dependent_slice_segment_flag)
  {
    ...
    LOG1("slice_type                           : %c\n",
         shdr->slice_type == 0 ? 'B' : shdr->slice_type == 1 ? 'P' : 'I');
                                ...
                                int x0, int y0, int ctDepth)
{
  ...
  if (availableL && get_ctDepth(ctx->img, ctx->current_sps, x0 - 1, y0) > ctDepth)
    condL = 1;
  if (availableA && get_ctDepth(ctx->img, ctx->current_sps, x0, y0 - 1) > ctDepth)
    condA = 1;
                               ...
                               int x0, int y0, int ctDepth)
{
  ...
  if (availableL && get_cu_skip_flag(ctx->current_sps, ctx->img, x0 - 1, y0))
    condL = 1;
  if (availableA && get_cu_skip_flag(ctx->current_sps, ctx->img, x0, y0 - 1))
    condA = 1;
                        ...
                        int xC, int yC, int xN, int yN)
{
  ...
  if (xN >= ctx->current_sps->pic_width_in_luma_samples)
  {
    return 0;
  }
  if (yN >= ctx->current_sps->pic_height_in_luma_samples)
  {
    return 0;
  }
                    ...
                    int cIdx)
{
  ...
  for (int i = lastSubBlock; i >= 0; i--)
  {
    ...
    {
      if (S.x > 0)
        coded_sub_block_neighbors[S.x - 1 + S.y * sbWidth] |= 1;
      if (S.y > 0)
        coded_sub_block_neighbors[S.x + (S.y - 1) * sbWidth] |= 2;
    }
                        ...
                        int cbf_luma, int cbf_cb, int cbf_cr)
{
  ...
  if (cbf_luma || cbf_cb || cbf_cr)
  {
    ...
    if (cbf_luma)
    {
      if ((err = residual_coding(ctx, tctx, x0, y0, xL, yL, log2TrafoSize, 0)) != DE265_OK)
        return err;
    }
    ...
    if (log2TrafoSize > 2)
    {
      if (cbf_cb)
      {
        if ((err = residual_coding(ctx, tctx, x0, y0, xL, yL, log2TrafoSize - 1, 1)) != DE265_OK)
          return err;
      }

      if (cbf_cr)
      {
        if ((err = residual_coding(ctx, tctx, x0, y0, xL, yL, log2TrafoSize - 1, 2)) != DE265_OK)
          return err;
      }
    }
    else if (blkIdx == 3)
        ...
        for (int j = 0; j < nCbS; j += pbOffset)
          for (int i = 0; i < nCbS; i += pbOffset)
          {
            ...
            else
            {
              ...
              for (int n = 0; n <= 2; n++)
              {
                if (IntraPredMode >= candModeList[n])
                {
                  IntraPredMode++;
                }
              }
...
de265_error read_slice_segment(decoder_context *ctx, thread_context *tctx)
{
  ...
  decode_substream(tctx, false, -1, -1);
