...
de265_error de265_decode_NAL(de265_decoder_context* de265ctx, NAL_unit* nal)
{
  ...
  if (nal_hdr.nal_unit_type<32) {
    ...
    if (!continueDecoding) {
      ...
      for (int i=0;i<nal->num_skipped_bytes;i++) //can be parallelized
        {
          nal->skipped_bytes[i] -= headerLength;
        }

      for (int i=0;i<hdr->num_entry_point_offsets;i++) { //can be parallelized
      ...
      bool use_tiles = (ctx->num_worker_threads > 0 && // we need to check this condition
                        ctx->current_pps->tiles_enabled_flag);
       printf("%s tiles \n", use_tiles?"true":"false");
       // use_tiles=1;
      ...
      if (!use_WPP && !use_tiles) {
        ...
        increase_pending_tasks(ctx->img, nTiles);
         ...
         //parallel
          ...
          for (int ty=0;ty<pps->num_tile_rows;ty++) // this  is Y direction of tiles int ty private
          for (int tx=0;tx<pps->num_tile_columns;tx++) { //this is for x direction
            ...
            */
            ...
            ctx->thread_context[tile].shdr = hdr; // one tile------- from here to
            ...
            int dataEnd;
            if (tile==nRows-1) dataEnd = reader.bytes_remaining; //storage seems to be in array format 0 to n-1 for n elements
            else               dataEnd = hdr->entry_point_offset[tile];

            init_thread_context(&ctx->thread_context[tile]); // does this create new threads?
