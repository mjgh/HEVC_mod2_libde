...
int main(int argc, char** argv)
{
  ...
  struct timeval tv_start_frame; // gg add
  ...
  while (!stop)
    {
      ...
      gettimeofday(&tv_start_frame, NULL); // gg add
      ...
      while (more)
        {
          ...
          //struct timeval tv_end_frame; // gg add
		  //gettimeofday(&tv_end_frame, NULL); // gg add
		  //double secs_frame = tv_end_frame.tv_sec-tv_start_frame.tv_sec;
          //secs_frame += (tv_end_frame.tv_usec - tv_start_frame.tv_usec)*0.001*0.001;
		  //fprintf(stderr,"decoding time for Frame: %5.3f \n",secs_frame);
  ...
  fprintf(stderr,"total decoding time %5.3f \n",secs);
